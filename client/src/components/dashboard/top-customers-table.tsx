            ...
            {customers.map((customer) => (
              <TableRow key={customer.id} className="hover:bg-[#F8FAFC]">
                ...
                <TableCell className="text-right font-medium tabular-nums">
                  ${customer.revenue.toFixed(2)}M
                </TableCell>
                <TableCell className="text-right tabular-nums">
                  ${customer.assets.toFixed(1)}B
                </TableCell>
                <TableCell className="text-right">
                  <span className={`inline-flex items-center ${customer.changePercent >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                    <i className={`ri-arrow-${customer.changePercent >= 0 ? 'up' : 'down'}-line mr-1`}></i>
                    {Math.abs(customer.changePercent).toFixed(1)}%
                  </span>
                </TableCell>
              </TableRow>
            ))}
